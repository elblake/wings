...
from_script_menu(_Mode) ->
    [{?__(4,"Script-based Commands..."), {command_from_script, select}}].
...
run_script_getting_data_special(QueryState, ShowTupleDebug, SendPID, Ret_1) ->
    case run_script_tuplefy(Ret_1) of
        ...
        [['%we!', [Mod,FunName|ApplyArgs]]|_]
          when is_list(ApplyArgs) ->
            {Ret,QueryState_1} = change_we(Mod,FunName,ApplyArgs,QueryState,erlang:get(scripting_callable)),
            SendPID ! {reply_to_script, Ret};

        [['%we', [Mod,FunName|ApplyArgs]]|_]
          when is_list(ApplyArgs) ->
            QueryState_1 = QueryState,
            Ret = val_from_we(Mod,FunName,ApplyArgs,QueryState,erlang:get(scripting_callable)),
            SendPID ! {reply_to_script, [Ret]};
...
-spec load_lang_file(filename:all(), string()) -> {ok, list()} | error.
...
wscr_command(Cont_0) ->
    ...
    CmdChanges = string:tokens(CmdChanges_0, " ,"),
    CmdInputs_0 = map_find(command_inputs, Cont, fun () -> CmdChanges_0 end),
    CmdInputs = string:tokens(CmdInputs_0, " ,"),
    {wscr_command_1(CmdInputs), gb_sets:from_list(wscr_command_1(CmdChanges))}.
wscr_command_1(List) ->
    wscr_command_1(List, []).
