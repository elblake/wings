...
set_face_attr_vs(Type, Face, NewList, #we{fs=Ftab}=We0) ->
    Edge = gb_trees:get(Face, Ftab),
    set_face_attr_vs(Type, Face, Edge, Edge, lists:reverse(NewList), We0).
set_face_attr_vs(_Type, _, LastEdge, LastEdge, [], We) -> We;
set_face_attr_vs(Type, Face, Edge, LastEdge, [Val|NewList], #we{es=Etab}=We0) ->
    case array:get(Edge, Etab) of
        #edge{lf=Face,ltsu=NextEdge} ->
            set_face_attr_vs(Type, Face, NextEdge, LastEdge, NewList, 
                set_face_attr_vs_1(Type, Edge, Face, Val, We0));
        #edge{rf=Face,rtsu=NextEdge} ->
            set_face_attr_vs(Type, Face, NextEdge, LastEdge, NewList, 
                set_face_attr_vs_1(Type, Edge, Face, Val, We0))
    end.
